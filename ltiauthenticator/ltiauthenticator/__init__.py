...
class LTIAuthenticator(Authenticator):
    ...
    def authenticate(self, handler, data=None):
        ...
        if validator.validate_launch_request(
            ...
            # Shenandoah pharmacy canvas does not return the custom user id, so this pulls the user's email and removes the @su.edu
            user_id = handler.get_body_argument('lis_person_contact_email_primary').split('@')[0]
